...
BOOST_FIXTURE_TEST_CASE( Evaluate_overrides_HoldAll, BuiltinFunctionFixture ) {

	addAttribute("f", "HoldAll");

	boost::optional<ast::Node> result = parseAndEvaluate("f[Evaluate[1+1], 1+1]");

	BOOST_REQUIRE(result);

	ast::Node expected = evaluate(ast::Node::make<ast::FunctionCall>("f", {ast::Node::make<math::Rational>(2), ast::Node::make<ast::FunctionCall>("Plus", {ast::Node::make<math::Rational>(1), ast::Node::make<math::Rational>(1)})}));

	BOOST_CHECK_EQUAL( *result, expected );
}


BOOST_FIXTURE_TEST_CASE( Evaluate_overrides_HoldFirst, BuiltinFunctionFixture ) {
...
BOOST_FIXTURE_TEST_CASE( Evaluate_overrides_HoldRest, BuiltinFunctionFixture ) {

	addAttribute("f", "HoldRest");

	boost::optional<ast::Node> result = parseAndEvaluate("f[1+1, Evaluate[1+1]]");

	BOOST_REQUIRE(result);

	ast::Node expected = evaluate(ast::Node::make<ast::FunctionCall>("f", {ast::Node::make<ast::FunctionCall>("Plus", {ast::Node::make<math::Rational>(1), ast::Node::make<math::Rational>(1)}), ast::Node::make<math::Rational>(2)}));
